...
def conversion_liens(base_url, headers, collection_id, chemin_collection):
    """Convertit des liens internes pour être compatible avec Outline afin de permettre une navigation entre documents"""
    ...
    for document in extraction_data_liste:
        ...
        if correspondance_tatouage:
            ...
            chemin_relatif = os.path.relpath(chemin_originel,os.path.normpath(chemin_collection)).replace("\\", "/")
...
#Début de la boucle principale afin d'importer et de modifier nos documents présent localement dans un format qui est supporté par Outline tout en gardant l'arborescence ainsi que la structure interne des documents.
